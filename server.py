...
Prima installazione (una volta sola):
  pip3 install yfinance scipy numpy statsmodels

  (pandas e certifi arrivano insieme a yfinance; statsmodels no e va
   installato a parte: serve alle regressioni CAPM e Fama-French)
...
import json
import os 
import sys                                                            #chiude il programma se mancano le librerie richieste
import math
import numpy as np

PORT = 7532             #rappresenta il numero di porta su cui il server ascolta le richieste HTTP. Può essere cambiato se la porta è già in uso.
...
# statsmodels serve alle regressioni CAPM e Fama-French. Non arriva con
# yfinance ne' con scipy, quindi va verificato qui: senza questo controllo
# l'assenza si manifesterebbe come errore 500 al primo clic dell'utente,
# invece che come messaggio chiaro all'avvio.
try:
    import statsmodels.api as _sm_check
    del _sm_check
except ImportError:
    print("\n  ERRORE: pip3 install statsmodels\n")
    sys.exit(1)

print("  [OK] yfinance + scipy + statsmodels pronti")
...
def search_tickers(query):
    try:
        ...
        quotes = []                                                    #il tuo cestino vuoto
        for q in (results.quotes or []):                               #i dati di yahoo (results.quotes) sono una lista di dizionari, ognuno con info su un titolo
            if q.get('quoteType') in ('CURRENCY', 'OPTION'):           #ricorda che q è un dizionario, quindi usa get() per ottenere i valori delle chiavi. Se il tipo di titolo è CURRENCY o OPTION, salta questo titolo e passa al prossimo
                continue
            quotes.append({
                'symbol':    q.get('symbol', ''),
                'longname':  q.get('longname') or q.get('shortname', ''),
                'shortname': q.get('shortname', ''),
                'exchDisp':  q.get('exchDisp') or q.get('exchange', ''),    # nome display della borsa
                'quoteType': q.get('quoteType', ''),                        #tipo di titolo (es. EQUITY, ETF, INDEX)
            })
...
def get_quote(symbol):
    try:
        t = yf.Ticker(symbol)           #crea un oggetto Ticker per il simbolo specificato
        hist = t.history(period='5d')
        closes = hist['Close'].dropna()  # scarta bar incompleti (es. giornata odierna/weekend)
        if closes.empty:
            return {'error': f'Nessun dato per {symbol}'}
        price = float(closes.iloc[-1])     #la classe float riporta i numeri al tipo nativo di python, in modo da poterli usare nelle operazioni matematiche
        prev  = float(closes.iloc[-2]) if len(closes) > 1 else price
        chg   = (price - prev) / prev * 100 if prev else 0.0 #se prev è diverso da zero
        info  = t.fast_info
        return {
...
def get_chart(symbol):
    try:
        ...
        s      = hist['Close'].dropna()                             #series
        closes = [float(x) for x in s.tolist()]                     #lista
        ...
        dates  = [d.strftime('%Y-%m-%d') for d in s.index]          #lista
        info   = t.fast_info
        price  = closes[-1]
        return {
            'chart': {'result': [{'meta': {
...
def align_returns(symbols_data):
    ...
    """
    have_dates = bool(symbols_data) and all(                        #bool ci assicura che la condizione sia True o False, non None
        d.get('dates') and len(d['dates']) == len(d.get('closes') or []) #all restituisce true se tutti i valori sono veri, se anche uno è falso 
        for d in symbols_data                                            #si ripiega sull'allineamento per posizione
    )                                                                    #for produce sequenza di valori true/false, all li valuta
...
def portfolio_stats(w, mu, cov):
    r = float(w @ mu)
    v = float(np.sqrt(w @ cov @ w))     #è la radice quadrata della varianza, quindi la deviazione standard
    return r, v
...
def optimize_markowitz(symbols_data, rf):
    ...
    w0 = np.ones(n) / n                     #punto di partenza per l'ottimizzazione: pesi uguali
    ...
    eq_constraint = {'type': 'eq', 'fun': lambda w: np.sum(w) - 1}          #vincolo di somma dei pesi = 1
    ...
    # Il range deve arrivare oltre la tangenza con short: con la leva quel
    # portafoglio puo' rendere molto piu' del miglior titolo singolo, e con un
    # tetto fissato a ret_max*1.2 la curva si fermerebbe prima del punto,
    # facendolo sembrare fuori dalla frontiera.
    # Sul lato basso si sottrae una quota del valore assoluto: 'ret_min * 0.8'
    # con ret_min negativo alzerebbe il limite invece di abbassarlo.
    lo_short = ret_min - 0.2 * abs(ret_min)
    hi_short = max(ret_max * 1.2, r_ts * 1.15)
    frontier_short = []
    for target in np.linspace(lo_short, hi_short, 60):
        cons = [
            eq_constraint,
            {'type': 'eq', 'fun': lambda w, t=target: float(w @ mu) - t}
        ]
    ...
    return {
        ...
        'asset_rets': [round(float(x) * 100, 3) for x in mu],           #x = i valori, uno per volta
        'asset_vols': [round(float(np.sqrt(cov[i,i])) * 100, 3) for i in range(n)], #i = 0, 1, 2, ... n-1 le posizioni
        'rf': round(rf * 100, 3),
...
def calc_diversification(symbols_data, rf):
    ...
    Sw     = cov @ w                          #Sw è il vettore dei contributi marginali al rischio di ciascun asset
    var_p  = float(w @ cov @ w)
    mctr   = [float(w[i] * Sw[i] / var_p * 100) if var_p > 0 else 0 for i in range(n)]          #quota della varianza di portafoglio attribuibile a ciascun asset
    ...
    for i in range(n):
        ...
        avg_corr = float((corr[i].sum() - 1) / (n - 1)) if n > 1 else 0 #-1 esclude la diagonale, più il valore è prossimo a 0 e più è buono per diversificare il portafoglio. 
        if n > 2:
            idx = [j for j in range(n) if j != i]                       #leave one out: rimuove il titolo i e calcola la tangenza sugli altri n-1 titoli
            _, _, _, sharpe_wo = _tangency_long(mu[idx], cov[np.ix_(idx, idx)], rf) #cov[np.ix_(idx, idx)] estrae la sotto-matrice della covarianza corrispondente agli indici idx  
        else:
    ...
    for d in symbols_data:
        ...
        try:
            ...
            if len(h) >= 120:
                ...
                dates  = [ts.strftime('%Y-%m-%d') for ts in h.index.tz_localize(None).normalize()] #tz.localize toglie il fusorario e normalize mette tutte le date a mezzanotte
        except Exception:
            pass
        if closes:
...
def calc_montecarlo(symbols_data, rf, years, capital, target, method, wmode, n_sims=3000):
    ...
    min_len = aligned.shape[1]              #il numero di rendimenti (sedute comuni -1), utile per capire quanto è affidabile la simulazione
    mu  = aligned.mean(axis=1) * 252
    cov = np.cov(aligned) * 252
    ...
    # Serie storica dei rendimenti giornalieri del portafoglio (pesi fissi) non è buy e hold, ogni sera si vende ciò che è salito e si compra ciò che è sceso per tornare ai pesi iniziali. 
    port_daily = w @ aligned                #(n, T)
    ann_ret = float(port_daily.mean() * 252)
    ann_vol = float(port_daily.std(ddof=1) * np.sqrt(252)) #ddof=1 mi dice di dividere per T-1, è la correzione di bessel, rende la stima non distorta. 

    T = int(252 * years)
    rng = np.random.default_rng()   #generatore di numeri casuali np 
    if method == 'normal':
    ...
    del sims
    finals = paths[:, -1].copy() #paths[:, -1] prende l'ultima colonna, il valore finale di ogni simulazione
    ...
    bands = {str(p): [round(float(x)) for x in np.percentile(sub, p, axis=0)]
             for p in (5, 25, 50, 75, 95)}          #dict comprehension: per ogni percentile p nella tupla (5, 25, 50, 75, 95) crea una voce del dizionario bands
    ...
    k = max(1, int(0.05 * n_sims))          #serve per il CvaR 5%, su 3000 simulazioni sono 150
    med = float(np.median(finals))          #mediana, ci servirà per 'median' e dentro il calcolo ci 'cagr_median'
    return {
        'symbols':  [d['symbol'] for d in symbols_data],
    ...
    try:
        ...
        if benchmark:
            try:
                ...
                if len(hb) >= 60:
                    series['__BENCH__'] = hb    #stringa del dizionario, non il simbolo reale, per evitare collisioni con titoli veri
                    bench_ok = True
            except Exception:
                pass

        df = pd.DataFrame(series).dropna() #allinea per data, scarta le sedute in cui manca almeno un titolo
        if len(df) < 60:
        ...
        if w.sum() <= 0:                 #se la somma è zero o  negativa
            w = np.ones(len(items))      #pesi uguali
        w = w / w.sum()                  #se la somma è positiva restano normali
        ...
        end_vals = shares * P[-1]              # P[-1] è il vettore dei prezzi l'ultimo giorno
        w_end = end_vals / end_vals.sum()      

        r = nav[1:] / nav[:-1] - 1             #rendimenti giornalieri del portafoglio, sfasamento come in calc_daily_returns
        n_years = len(r) / 252
        total = float(nav[-1] / 100 - 1)
        cagr  = float((nav[-1] / 100) ** (1 / n_years) - 1) if n_years > 0 else 0
        vol   = float(np.std(r, ddof=1) * np.sqrt(252))     #delta degrees of freedom = 1 ; divido per T-1
        ...
        lim = max(starts, key=lambda k: starts[k])                      #key restituisce il simbolo con la data di inizio più recente
        ...
        if bench_ok:
            ...
            bench_nav = b / b[0] * 100      #normalizziamo il beenchmarck a 100 il primo giorno, così come il portafoglio
...
def calc_dividends(symbol, period):
    ...
    try:
        ...
        common = close.index.intersection(adj.index)            #altro metodo di allineamento per data, intersection restituisce le date comuni tra close e adj
        if len(common) < 60:
        ...
        price_growth = close / close.iloc[0]                        #dividere per il primo valore normalizza a 1
        total_growth = adj / adj.iloc[0]
        n_years = len(close) / 252

        price_ret = float(price_growth.iloc[-1] - 1)                #es. 1.2 - 1 = 0.2, quindi 20% di crescita del prezzo
        ...
        cutoff = close.index[-1] - pd.Timedelta(days=365)           #un anno fa rispetto all'ultimo giorno di chiusura
        ttm_sum = float(divs[divs.index >= cutoff].sum())           #TTM = trailing twelve months, quindi somma dei dividendi degli ultimi 12 mesi
        ...
        years = [{'year': int(y), 'amount': round(float(a), 4),
                 ...
                 for y, a in per_year.items()] #y anno, a somma dei dividendi di quell'anno
        ...
        if len(full) >= 2 and full[0]['amount'] > 0:
            ...
            if span > 0: #l'intervalo di anni pieni deve essere almeno 1, altrimenti non ha senso calcolare il CAGR
                div_cagr = round(((full[-1]['amount'] / full[0]['amount']) ** (1 / span) - 1) * 100, 2)
            cuts = sum(1 for i in range(1, len(full)) if full[i]['amount'] < full[i-1]['amount'] * 0.999) #tolgo 0.1% per evitare falsi positivi da arrotondamenti, cuts conta le riduzioni di dividendo tra anni pieni
...
def calc_backtest(symbols, rf, hist, lookback_days, rebal_days, benchmark):
    ...
    rets = df.pct_change().dropna() #dropna scarta la prima riga che è NaN
    n = len(symbols)
    ...
    t = lookback_days
    while t < T:
        win = R[t - lookback_days:t]                #il periodo di stima non contiene nemmeno un gionro del periodo valutato, quindi è out-of-sample, altrimenti avremmo look-ahead bias
        ...
        end = min(t + rebal_days, T) #impedisce di andare oltre la fine della serie, l'ultimo segmento può essere più corto di rebal_days
        seg = R[t:end] #t incluso, end escluso
        for name, w in (('tangency', w_tan), ('minvar', w_mv), ('equal', w_eq)):
            strat[name].extend((seg @ w).tolist()) #rendimenti giornalieri del portafoglio per il segmento successivo, to list poi convertiamo in lista python perché stat[name] è una lista python, non un array numpy
        for name, w in (('tangency', w_tan), ('minvar', w_mv)):
            if w_prev[name] is not None:
                turnover[name].append(float(np.abs(w - w_prev[name]).sum() / 2))
            w_prev[name] = w
        n_rebal += 1        #quanti ribilanciamenti sono stati fatti
        t = end             #fino a quando t<T, fino a quando non abbiamo finito di ribilanciare tutta la serie storica il ciclo while continua a ribilanciare 

    eval_dates = dates[lookback_days:]
    metrics = {} #dizionario per memorizzare le metriche di ogni strategia
    ...
    if bench_ok:
        curves['bench'] = build_curve('bench', bench_r[lookback_days:].tolist()) #confronta con il benchmark solo sulle date effettivamente valutate. 
    ...
    return {
        ...
        'turnover': {k: round(float(np.mean(v)) * 100, 1) if v else 0.0 for k, v in turnover.items()}, #media dei turnover e converte in percentuale
        'dates':  [eval_dates[i] for i in idx],
        'equity': {name: [round(float(c[i]), 4) for i in idx] for name, c in curves.items()},
        'metrics': metrics,   # popolato da build_curve: total, cagr, vol, sharpe, maxdd per strategia
    }
...
def calc_risk(symbol, period, rf):
    ...
    try:
        ...
        dd_dev   = float(np.sqrt(np.mean(downside ** 2)) * np.sqrt(252))   #downside deviation = deviazione standard dei rendimenti negativi rispetto al risk-free, annualizzata 
        ...
        calmar   = ann_ret / abs(max_dd) if max_dd < 0 else 0           #calmar ratio = rapporto tra rendimento annuo e drawdown massimo, misura il rischio di perdita massima rispetto al rendimento atteso
        ...
        skew = float(stats.skew(dr))      #se negativa indica che la coda sinistra è più lunga della coda destra, quindi ci sono più rendimenti negativi estremi rispetto a quelli positivi (asimmetria negativa)
...
def calc_capm(symbol, benchmark, period, rf):
    ...
    try:
        ...
        r1 = h1.resample('ME').last().pct_change().dropna()     #ME = month end, raggruppa per mese e prende l'ultimo valore del mese, pct_change calcola il rendimento percentuale mese su mese, dropna scarta eventuali NaN
        ...
        X = sm.add_constant(e2)     #aggiunge una colonna di 1 per stimare l'intercetta (alpha)
        nw_lags = max(1, int(np.floor(4 * (n / 100) ** (2 / 9))))   # Newey-West (1994)
        fit = sm.OLS(e1, X).fit(cov_type='HAC',     #il modello prima vuole la variabile dipendente (e1) e poi  i regressori (X), fit() stima i parametri, cov_type='HAC' indica di usare errori standard robusti HAC (Newey-West), 
                                cov_kwds={'maxlags': nw_lags, 'use_correction': True})  #maxlags = numero di lag da considerare per la correzione Newey-West, use_correction = True applica la correzione di Small (1980) per campioni piccoli
                                                                                        # 4 * (n / 100) ** (2 / 9) è la formula di Newey-West per determinare il numero ottimale di lag in base alla dimensione del campione n  
        alpha_monthly, beta = float(fit.params[0]),  float(fit.params[1])       #0 = alpha, 1 = beta
        ...
        return {
            ...
            'monthly_market': [round(float(x), 5) for x in r2],     #servono a disegnare la Security Market Line (SML) e la Security Characteristic Line (SCL)
            'n_months':       n,
        }
    except Exception as e:
        import traceback; traceback.print_exc()
...
import io, zipfile, ssl, certifi #io=tratta i dati in memoria come file, zipfile=per leggere archivi zip, ssl=per connessione cifrata, certifi=per l'elenco aggiornato di autorità di certificazione (CA) affidabili
...
def fetch_ff_data(dataset):
    ...
    import time #serve per misurare il tempo di cache
    ...
    try:
        with urllib.request.urlopen(req, timeout=15, context=ctx) as r: #urlopen apre l'URL e restituisce un oggetto file-like, timeout=15 secondi, context=ctx per usare il contesto SSL con i certificati aggiornati
            raw = r.read()  #prende il contenuto della risposta HTTP come bytes
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        with z.open(fname) as f:
            text = f.read().decode('utf-8', errors='ignore')        #decode converte bytes in stringa, i caratteri non decodificabili vengono ignorati
    ...
    for line in text.split('\n'):
        ...
        if len(parts) >= 4 and len(parts[0]) == 6: # YYYYMM è una data mensile YYYY è anno
            try:
                date = int(parts[0])
                vals = [float(v)/100 for v in parts[1:] if v.strip() not in ('', 'RF')] #divido per 100 perché i dati di Ken French sono in percentuale, quindi li converto in decimali
                rows.append([date] + vals)                                              #v.strip() rimuove spazi bianchi, se il valore è vuoto o 'RF' lo ignoro, altrimenti lo converto in float e divido per 100
            except ValueError:
                continue
    ...
    ncols = len(rows[0])                                                               #numero di colonne del dataset, che corrisponde al numero di valori in ciascuna riga
...
def calc_fama_french(symbol, model_type, period, rf_annual):
    ...
    try:
        ...
        X = sm.add_constant(np.column_stack(factors))      #aggiunge una colonna di 1 per stimare l'intercetta (alpha) e combina i fattori in una matrice (T, k) dove T è il numero di mesi e k è il numero di fattori
        ...
        alpha_m = res.params[0]             #intercetta = alpha mensile
        betas   = res.params[1:]            #tutti i beta
        pvals   = res.pvalues
        tstats  = res.tvalues
        r2      = res.rsquared
        ...
        return {
            ...
            'betas': {
                ...
                for i in range(len(factor_names))       #per ogni fattore, crea un dizionario con il valore del beta, p-value, t-stat e se è significativo
            },
            ...
            # mesi allineati: servono al grafico dei residui, che ha senso solo
            # in ordine temporale (l'autocorrelazione si vede solo cosi')
...
class Handler(http.server.BaseHTTPRequestHandler): 
    ...
    def do_GET(self): 
        ...
        try:
            ...
            self.send_json({'error': str(e)}, 500)      #4 colpa del client, 5 colpa del server
